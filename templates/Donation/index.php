		...
		<div class="dropdown mx-3 mt-2">
			...
			<i class="fa-solid fa-bars text-primary"></i>
				...
				<div class="dropdown-menu dropdown-menu-end" aria-labelledby="orederStatistics">
							<li><?= $this->Html->link(__('<i class="fa-solid fa-plus"></i> New Donation'), ['action' => 'add'], ['class' => 'dropdown-item', 'escapeTitle' => false]) ?></li>
							</div>
		</div>
</div>
</div>
	...
	<div class="table-responsive">
		...
		?>
                <tr>
                    ...
                    <th><?= $this->Paginator->sort('donor_nric') ?></th>
            ...
            <tbody>
				...
				<td><?php echo $counter++ ?></td>
                    ...
                    <td><?= h(substr($donation->donor_nric, 0, 6) . '****') ?></td>
					...
					<td class="actions text-center">
						<div class="btn-group shadow" role="group" aria-label="Basic example">
							<?= $this->Html->link(__('<i class="far fa-folder-open"></i>'), ['action' => 'view', $donation->id], ['class' => 'btn btn-outline-primary btn-xs', 'escapeTitle' => false]) ?>
							<?= $this->Html->link(__('<i class="fa-regular fa-pen-to-square"></i>'), ['action' => 'edit', $donation->id], ['class' => 'btn btn-outline-warning btn-xs', 'escapeTitle' => false]) ?>
			...
			<div class="row pb-3">
				<div class="col-md-4">
				  <div class="stat_card card-1 bg-body-tertiary">
					<h3><?php echo $total_donation; ?></h3>
					<p>Total Donation</p>
				  </div>
				</div>
				<div class="col-md-4">
				  <div class="stat_card card-2 bg-body-tertiary">
					<h3><?php echo $total_donation_active; ?></h3>
					<p>Active Donation</p>
				  </div>
				</div>
				<div class="col-md-4">
				  <div class="stat_card card-3 bg-body-tertiary">
					<h3><?php echo $total_donation_archived; ?></h3>
					<p>Archived Donation</p>
				  </div>
				</div>
			</div>
...
<div class="row">
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
			<div class="card-title mb-0">Donation (Monthly)</div>
...
const monthly = new Chart(ctx, {
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
		<div class="card-title mb-0">Donation by Status</div>
			<div class="tricolor_line mb-3"></div>
		<div class="chart-container" style="position: relative;">
...
const status = new Chart(ctx_2, {
    ...
    data: {
        ...
        datasets: [{
            label: '# of Donation(s)',
			data: [<?= json_encode($total_donation_active); ?>, <?= json_encode($total_donation_disabled); ?>, <?= json_encode($total_donation_archived); ?>],
            backgroundColor: [
...
<div class="card bg-body-tertiary border-0 shadow mb-4">
	<div class="card-body">
		<div class="card-title mb-0">Search</div>
			<div class="tricolor_line mb-3"></div>
			<?php echo $this->Form->create(null, ['valueSources' => 'query', 'url' => ['controller' => 'Donation','action' => 'index']]); ?>
				<fieldset>
					<div class="mb-1"><?php echo $this->Form->control('',['required' => false]); ?></div>
				</fieldset>
